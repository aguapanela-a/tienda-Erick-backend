...
public class JwtServicio {
    ...
    //metodo para genrar el accessToken a partir de un usuario, el accessToken se genera con los datos del usuario (Claims) y se firma con la llave secreta (Secret Key) con un algoritmo de firma (HS256)
    public String generateToken(User usuario, String tipoToken) throws JOSEException {
        ...
        // en este caso los datos son el username, id, nombre y numero de telefono del usuario, y la fecha de expiracion del accessToken
        JWTClaimsSet claimsSet = new JWTClaimsSet.Builder()
                .subject(usuario.getNombre())
                .claim("id", usuario.getId().toString())
                .claim("nombre", usuario.getNombre())
                .claim("numeroTelefono", usuario.getNumeroTelefono())
                .claim("tipoToken", tipoToken)
                .expirationTime(new Date(System.currentTimeMillis() + ("refresh".equals(tipoToken) ? refreshExpiracion : expiracion)))
                .build();
    ...
    public boolean esTokenRefresh(String token) throws ParseException {
        //parsea el accessToken para obtener el tipo de accessToken
        SignedJWT jwt = SignedJWT.parse(token);
        String tipoToken = jwt.getJWTClaimsSet().getStringClaim("tipoToken");
        return "refresh".equals(tipoToken);
    }

    //metodo para validar el tokem
    public String extraerTelefono(String token) throws JOSEException, ParseException {
        //parsea el accessToken para obtener el numero de telefono
        SignedJWT jwt = SignedJWT.parse(token);

        //Creamos un objeto verificador JWSVerifier con la llave secreta
        JWSVerifier verifier = new MACVerifier(secretKey.getBytes());

        //validar que el accessToken sea válido
        ...
        //retornar el numero de telefono del usuario que se encuentra en los claims del accessToken
